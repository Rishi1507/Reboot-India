...
import {
  buildAdminOtpEmail,
  buildBatchMessageEmail,
  buildCouponShareEmail,
...
router.post("/login", async (req, res) => {
  ...
  if (!admin) {
    if (
      ...
      admin = await prisma.adminUser.create({ data: { email, passwordHash: hash } });
    } else {
  ...
  const record = await prisma.adminLoginOtp.create({
    data: {
  ...
  await sendEmail({
    ...
    html: buildAdminOtpEmail({ otp, expiresMinutes }),
  });
...
router.post("/login/verify-otp", async (req, res) => {
  ...
  const record = await prisma.adminLoginOtp.findUnique({
    where: { id: String(otpId) },
    include: { adminUser: true },
  });
  if (!record) return res.status(401).json({ error: "Invalid OTP" });
  if (record.usedAt) return res.status(401).json({ error: "OTP already used" });
  if (record.expiresAt.getTime() < Date.now()) return res.status(401).json({ error: "OTP expired" });
...
async function isMasterAdmin(adminId: string) {
  const masterId = String(process.env.MASTER_ADMIN_ID || "").trim();
  if (masterId && masterId === adminId) return true;

  const masterEmail = String(
    process.env.MASTER_ADMIN_EMAIL || process.env.ADMIN_BOOTSTRAP_EMAIL || "",
  )
    .trim()
    .toLowerCase();
...
router.post("/admin-users", async (req: any, res) => {
...
router.post("/admin-users/:id/reset-password", async (req: any, res) => {
  if (!(await isMasterAdmin(req.adminId))) return res.status(403).json({ error: "Forbidden" });
  const password = String(req.body?.password || "").trim();
  if (!password || password.length < 8) {
    return res.status(400).json({ error: "Password must be at least 8 characters" });
...
router.delete("/admin-users/:id", async (req: any, res) => {
  if (!(await isMasterAdmin(req.adminId))) return res.status(403).json({ error: "Forbidden" });
...
router.delete("/departures/:id", async (req, res) => {
  ...
  const departure = await prisma.departure.delete({ where: { id: req.params.id } });
  res.json(departure);
});
...
router.get("/bookings/pending-full-payment", async (_, res) => {
  ...
  const bookings = await prisma.booking.findMany({
    where: {
      amountDue: { gt: 0 },
      paymentStatus: { in: ["ADVANCE_PAID", "PENDING_ADVANCE"] as any },
...
router.post("/bookings/:id/send-payment-reminder", async (req, res) => {
  try {
    ...
    if (booking.amountDue <= 0) return res.status(400).json({ error: "No due amount" });
...
router.post("/departures/:id/manage-batch/send-message", async (req, res) => {
  try {
    ...
    const where: any = { departureId: req.params.id };
    if (Array.isArray(bookingIds) && bookingIds.length > 0) where.id = { in: bookingIds };

    const bookings = await prisma.booking.findMany({
      where,
      include: { customer: true },
    });
    if (!bookings.length) return res.status(404).json({ error: "No bookings found in batch" });
...
router.post("/departures/:id/send-payment-reminders", async (req, res) => {
  try {
    const departure = await prisma.departure.findUnique({ where: { id: req.params.id } });
...
router.get("/trek-blogs", async (req, res) => {
  const { trekId, status, featured } = req.query as Record<string, string>;
...
router.get("/trek-blogs/:id", async (req, res) => {
  const blog = await prisma.trekBlog.findUnique({
    where: { id: req.params.id },
    include: { trek: true },
  });
...
router.post("/trek-blogs", async (req, res) => {
  try {
    ...
    if (!payload.trekId) return res.status(400).json({ error: "trekId is required" });
    if (!payload.title) return res.status(400).json({ error: "title is required" });
    if (!payload.shortIntro) return res.status(400).json({ error: "shortIntro is required" });
    ...
    const exists = await prisma.trekBlog.findUnique({ where: { slug } });
    if (exists) slug = `${slug}-${Date.now().toString().slice(-4)}`;

    const blog = await prisma.trekBlog.create({
      data: {
...
router.put("/trek-blogs/:id", async (req, res) => {
  try {
    ...
    if (slug) {
      const existing = await prisma.trekBlog.findFirst({
        where: { slug, NOT: { id: req.params.id } },
      });
      if (existing) slug = `${slug}-${Date.now().toString().slice(-4)}`;
    }

    const blog = await prisma.trekBlog.update({
      where: { id: req.params.id },
      data: {
        trekId: payload.trekId,
        title: payload.title,
        ...
        permitsRequired:
          payload.permitsRequired !== undefined ? Boolean(payload.permitsRequired) : undefined,
        ...
        showOnHomepage:
          payload.showOnHomepage !== undefined ? Boolean(payload.showOnHomepage) : undefined,
      },
      include: { trek: true },
    });
    res.json(blog);
...
router.delete("/trek-blogs/:id", async (req, res) => {
  await prisma.trekBlog.delete({ where: { id: req.params.id } });
  res.json({ success: true });
});

router.get("/trek-reviews", async (req, res) => {
  const { trekId, rating, status, featured } = req.query as Record<string, string>;
...
router.post("/trek-reviews", async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload.trekId) return res.status(400).json({ error: "trekId is required" });
    if (!payload.reviewerName) return res.status(400).json({ error: "reviewerName is required" });
    if (!payload.reviewText) return res.status(400).json({ error: "reviewText is required" });
...
router.put("/trek-reviews/:id", async (req, res) => {
  try {
    const payload = req.body || {};
    const review = await prisma.trekReview.update({
      where: { id: req.params.id },
      data: {
        trekId: payload.trekId,
        reviewerName: payload.reviewerName,
...
router.delete("/trek-reviews/:id", async (req, res) => {
  await prisma.trekReview.delete({ where: { id: req.params.id } });
  res.json({ success: true });
});
  ...
  try {
    ...
    const coupon = await prisma.coupon.update({
      ...
      data: {
        ...
        maxUsesPerEmail:
          data.maxUsesPerEmail !== undefined ? toNumber(data.maxUsesPerEmail) : undefined,
...
router.post("/coupons/:id/share", async (req, res) => {
  try {
    ...
    const terms: string[] = [];
    if (coupon.minAmount) terms.push(`Minimum booking amount: â‚¹${coupon.minAmount}`);
    if (coupon.maxUsesPerEmail) terms.push(`Max uses per email: ${coupon.maxUsesPerEmail}`);
    if (coupon.maxUses) terms.push(`Total max uses: ${coupon.maxUses}`);
    const html = buildCouponShareEmail({
...
router.post("/faqs", async (req, res) => {
  try {
    ...
    const faq = await prisma.pageFaq.create({
      data: {
        pageKey,
        question,
        answer,
...
router.put("/faqs/:id", async (req, res) => {
  try {
    const payload = req.body || {};
    const faq = await prisma.pageFaq.update({
      where: { id: req.params.id },
      data: {
...
router.delete("/faqs/:id", async (req, res) => {
  await prisma.pageFaq.delete({ where: { id: req.params.id } });
  res.json({ success: true });
});
