...
router.post("/treks", async (req, res) => {
    try {
        let { slug, syncDeparturePrices, gallery, itinerary, originalPrice, discountedPrice, ...data } = req.body;
        if (!slug)
...
router.put("/treks/:id", async (req, res) => {
    try {
        ...
        if (syncDeparturePrices) {
            ...
            if (newPrice) {
                await db_1.prisma.departure.updateMany({
...
router.delete("/departures/:id", async (req, res) => {
    const count = await db_1.prisma.booking.count({
