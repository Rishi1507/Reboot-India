...
router.post("/", async (req, res) => {
    try {
        ...
        let coupon = null;
        if (couponCode) {
            coupon = await db_1.prisma.coupon.findUnique({
        ...
        const discountAmountRaw = coupon && coupon.type === "PERCENT"
...
router.post("/:id/cancel", async (req, res) => {
    try {
        ...
        const updated = await db_1.prisma.$transaction(async (tx) => {
            ...
            if (!booking)
                return null;
            const matches = booking.customer.email.toLowerCase() === emailOrPhone ||
        ...
        if (updated.booking && updated.creditGranted >= 0) {
            ...
            const html = (0, email_1.buildBookingCancellationEmail)({
